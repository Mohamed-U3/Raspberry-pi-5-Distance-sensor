...
# Create I2C interface and initialize the SH1106 OLED display
...
# Function to measure distance and update the GUI and OLED display
def measure_distance():
    distance = int(sensor.distance * 100)
    # Update the Tkinter label with distance
    if distance < 20:
        distance_label.config(fg="red", text="Distance: {} cm\nHi!".format(distance))
    elif distance > 50:
        distance_label.config(fg="blue", text="Distance: {} cm\nBye!".format(distance))
    else:
        distance_label.config(fg="green", text="Distance: {} cm\nHello".format(distance))

    # Display distance on OLED
    with canvas(device) as draw:
        draw.rectangle((0, 0, device.width, device.height), outline=0, fill=0)  # Clear the display
        draw.text((10, 10), f"Distance: {distance} cm", fill="white")  # Display distance on OLED
